...
Start with `vidiq-get-started` for connection, account, channel, and credit guidance. Use
`vidiq-status` for diagnostics or interrupted asynchronous work. Use `vidiq-next-video-planner`
...
packaging, trends, competitor tracking, and Shorts inspiration to the matching `vidiq-*` skill. For
